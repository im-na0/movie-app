...
function routeRender(routes: Routes) {
  ...
  const query = queryString
    ...
    .reduce((acc, cur) => {
      ...
      acc[key] = value
      return acc
    }, {} as Query) // MEMO: 타입단언 사용!
  history.replaceState(query, '') // (상태, 제목)

  // 2) 현재 라우트 정보를 찾아서 렌더링!
  const currentRoute = routes.find(route => new RegExp(`${route.path}/?$`).test(hash))
  if (routerView) { // MEMO: 타입가드 사용!
    routerView.innerHTML = ''
    currentRoute && routerView.append(new currentRoute.component().el) // MEMO: &&로 타입가드 !
  }
...
interface SubscribeCallback {
  (arg: unknown): void // MEMO: 호출 시그니처 !
}
export class Store<S> { // MEMO: 여러 타입이 올 수 있는 제네릭 사용 !
  public state = {} as S // 상태(데이터) MEMO: 타입 변수에 따라 state 타입도 정의 !
